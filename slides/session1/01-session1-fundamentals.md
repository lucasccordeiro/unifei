...
Ask the same tool the *opposite* question — "can any input get past the
check?" — and it hands back one that does, as a test you can run.
...
ESBMC found the input that reaches the protected branch. The test compiles,
runs, and prints **"Access Granted with input SMT"** — no one had to guess it.
...
- Eight distinct letters — S E N D M O R Y. The words are fixed; what we
  solve for is **the digit hiding behind each letter**.
...
- The solver doesn't guess blindly: it fixes **M = 1**, then propagates
...
- **CompCert** — a C compiler with a proof that compilation preserves
  semantics; six CPU-years of fuzzing found no wrong-code bugs in its
  verified core, while every other compiler tested failed
...
4. Which technique proved the A340 flight-control code free of runtime
   errors?
