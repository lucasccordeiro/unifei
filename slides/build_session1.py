...
add_body(s, [
    "Ask the same tool the opposite question — \"can any input get past "
    "the check?\" — and it hands back one that does, as a test you can run.",
    ("  char buf[4];                          /* input ESBMC chooses */",
     0, True),
    ("  for (int i = 0; i < 3; i++) buf[i] = nondet_char();", 0, True),
    ("  buf[3] = '\\0';", 0, True),
    ("  if (strcmp(buf, \"SMT\") == 0)", 0, True),
    ("    assert(0);             /* claim: this is unreachable */", 0, True),
    ("", 0, True),
    ...
    ("}", 0, True),
    "",
    "ESBMC found the input that reaches the protected branch. The test "
    "compiles, runs, and prints \"Access Granted with input SMT\" — no one "
    "had to guess it.",
], size=20)
...
add_body(s, [
    "The BMC pipeline you just saw hunts a counterexample (or proves up "
    "to a bound). Abstract interpretation proves P the other way — "
    "over-approximate every run at once: no inputs, no paths, no bound.",
    "",
    ("x = 0                 # x ∈ [0, 0]", 0, True),
    ("while x < 100:", 0, True),
    ("    x = x + 1         # x ∈ [1, 100]", 0, True),
    ("# fixpoint invariant:  x ∈ [0, 100]", 0, True),
    ("assert 0 <= x <= 100  # implied by the invariant → PROVED", 0, True),
    "",
    ("Compute the tightest fact the loop can't escape (a fixpoint); if it "
     "implies P, P holds for every run — no unrolling.", 1),
...
add_body(s, [
    ("Abstract domain — the facts you track instead of exact values. "
     ...
     "domain → more precise, but more costly.", 1),
    "",
    ("Widening (∇) — forces loops to converge. The loop ascends "
     "[0,0] → [0,1] → [0,2] → … and may never stop; widening sees the "
     "bound climbing and extrapolates it to [0, +∞). Termination, at the "
     "cost of precision.", 1),
    "",
    ("Narrowing (Δ) — recovers what widening overshot. Re-reading the "
     "guard x < 100 pulls [0, +∞) back to [0, 100]: precision restored, "
     "still terminating.", 1),
    "",
    "ESBMC ships it too — --interval-analysis — and can supply the "
    "invariant k-induction needs (Session 2, Lab 3).",
], size=20)
