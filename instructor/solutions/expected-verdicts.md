...
| `labs/lab3/contract.c` | `esbmc contract.c --replace-call-with-contract clamp` | **SUCCESSFUL** | modular: at the call site ESBMC *asserts* requires, *havocs* the frame, and *assumes* ensures, so the caller's `assert(r >= lo && r <= hi)` holds from the contract alone (body not unrolled). Enforce is the dual — *assume* requires, run body, *assert* ensures + frame. **NB:** `--enforce-contract` only checks a contract if the function is reachable — `main()` must call it, else it reports SUCCESSFUL vacuously. |
...
| `labs/lab1/stage3_ir_reproduce.py` | `python3 stage3_ir_reproduce.py` | exit 1 (`AssertionError`) | Plain-CPython reproducer (no ESBMC). Runs `f` on both witnesses: `x = 2⁶³−1` holds (prints, no error — machine-word artifact), `x = -1` raises `AssertionError: z >= 2`. Exit status 1 = the `--ir` counterexample reproduced end-to-end. |
